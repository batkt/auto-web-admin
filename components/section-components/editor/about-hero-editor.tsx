...
interface AboutHeroEditorProps {
  data: AboutHeroFormData;
  onDataChange: (data: AboutHeroFormData) => void;
  sectionId?: string;
}

const AboutHeroEditor = ({ data, onDataChange, sectionId }: AboutHeroEditorProps) => {
  const { register, handleSubmit, watch, setValue, control } = useForm<AboutHeroFormData>({
    defaultValues: data,
  });
  ...
  const onSubmit = async (values: AboutHeroFormData) => {
