...
type DonateTransparencyFormData = {
  title: TranslatedText;
  description: TranslatedText;
};

interface DonateTransparencyEditorProps {
  data: any;
  onDataChange: (updatedData: any) => void;
  sectionId?: string;
}

const DonateTransparencyEditor = ({
  data,
  onDataChange,
  sectionId,
}: DonateTransparencyEditorProps) => {
  const { register, handleSubmit, watch, setValue } = useForm<DonateTransparencyFormData>({
    defaultValues: data,
  });
  ...
  const onSubmit = async (values: DonateTransparencyFormData) => {
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <Input
                    ...
                    className={cn(
                      'mt-1',
                      errors[`title${lang === 'en' ? 'En' : 'Mn'}`] && 'border-red-500'
                    )}
                    placeholder="Гарчиг оруулах"
                  />
                  {errors[`title${lang === 'en' ? 'En' : 'Mn'}`] && (
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    className={cn(
                      'mt-1',
                      errors[`description${lang === 'en' ? 'En' : 'Mn'}`] && 'border-red-500'
                    )}
                    placeholder="Тайлбар оруулах"
                    rows={4}
                  />
                  {errors[`description${lang === 'en' ? 'En' : 'Mn'}`] && (
                    <p className="text-xs text-red-500 mt-1">Англи талбар заавал бөглөх</p>
                  )}
                </div>
              </div>
...
export default DonateTransparencyEditor;
